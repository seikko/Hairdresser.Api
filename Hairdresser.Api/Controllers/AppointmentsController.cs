    ...
    : Controller
{
    ...
    public async Task<IActionResult> Index(string? date, int? workerId, string? status, string? search)
    {
        ...
        var viewModel =
            await appointmentService.GetAppointmentsForIndexAsync(selectedDate, workerId, status, search);
    ...
    public async Task<IActionResult> Create(AppointmentCreateViewModel model)
    {
        ...
        var workerExists = await workerService.GetWorkerByIdAsync(model.WorkerId) != null;
        if (!workerExists)
        {
        ...
        var hasConflict = await appointmentService.HasSlotConflictAsync(model.WorkerId, date, time);
        if (hasConflict)
        {
        ...
        var appointment = await appointmentService.CreateAppointmentFromAdminAsync(
            model.PhoneNumber,
            model.CustomerName,
            model.WorkerId,
            date,
        ...
        return RedirectToAction(nameof(Index),
            new { date = appointment.AppointmentDate.ToString("yyyy-MM-dd"), workerId = appointment.WorkerId });
    }

    [HttpGet]
    public async Task<IActionResult> Edit(int id)
    {
        var appointment = await appointmentService.GetAppointmentWithDetailsAsync(id);
    ...
    public async Task<IActionResult> Edit(AppointmentEditViewModel model)
    {
        ...
        var hasConflict = await appointmentService.HasSlotConflictAsync(model.WorkerId, date, time, model.Id);
        if (hasConflict)
        {
            ModelState.AddModelError(string.Empty, "Bu çalışan için seçilen tarih/saatte zaten bir randevu var.");
            return View(model);
        ...
        var success = await appointmentService.UpdateAppointmentAsync(
            model.Id,
            model.WorkerId,
            date,
            time,
        ...
        var appointment = await appointmentService.GetAppointmentByIdAsync(model.Id);
        return RedirectToAction(nameof(Index),
            new { date = appointment!.AppointmentDate.ToString("yyyy-MM-dd"), workerId = appointment.WorkerId });
    }
    ...
    {
        var dateOk = DateOnly.TryParseExact(date, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None,
            out d);
        var timeOk =
            TimeOnly.TryParseExact(time, "HH:mm", CultureInfo.InvariantCulture, DateTimeStyles.None, out t);
    ...
    private static DateOnly ParseDateOrDefault(string? date, DateOnly fallback)
    {
        if (!string.IsNullOrWhiteSpace(date) &&
            DateOnly.TryParseExact(date, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None,
                out var d))
        {
            return d;
        }
